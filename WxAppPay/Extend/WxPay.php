<?php
namespace common\lib\WxAppPay\Extend;
...
//                        $baseUrl = urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].$_SERVER['QUERY_STRING']);
	...
	public function GetJsApiParameters($UnifiedOrderResult)
	{
		...
		$jsapi->SetPackage("prepay_id=" . $UnifiedOrderResult['prepay_id']);//设置订单详情扩展字符串
		$jsapi->SetSignType("MD5");
		$jsapi->SetPaySign($jsapi->MakeSign());
		$parameters = json_encode($jsapi->GetValues());//获取设置的值
		return $parameters;
	}
...
//		$jsapi->setPackage_app("Sign=WXPay");
	...
	{
		$urlObj["appid"] = \WxPayConfig::APPID;//微信公众号
		$urlObj["secret"] = \WxPayConfig::APPSECRET;//公众帐号secert
		$urlObj["code"] = $code;
		$urlObj["grant_type"] = "authorization_code";
		$bizString = $this->ToUrlParams($urlObj);//拼接签名
